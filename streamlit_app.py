...
Upload a new raw occupancy-sensor CSV -> get a 30-minute-ahead
...
Expects, in the same folder:
    occupancy_model_30min.pkl
    occupancy_model_30min_config.json
...
st.caption(
    ...
    "This runs the trained 30-minute-ahead occupancy model, then a 1R1C "
...
if uploaded_file is not None and model is not None:
    ...
    forecast_chart_df = pred_df.set_index('timestamp')[
        ['occupancy_now', 'occupancy_forecast_30min']
    ]
    st.line_chart(forecast_chart_df)

    if 'occupancy_actual_30min_later' in pred_df.columns and pred_df['occupancy_actual_30min_later'].notna().any():
        with st.expander("Forecast accuracy (this upload includes enough history to check)"):
            accuracy_df = pred_df.dropna(subset=['occupancy_actual_30min_later']).set_index('forecast_time')[
                ['occupancy_actual_30min_later', 'occupancy_forecast_30min']
            ]
