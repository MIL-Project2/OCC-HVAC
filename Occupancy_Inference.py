...
Inference-only entry point for the 30-minute-ahead occupancy model.
...
DEFAULT_MODEL_PATH = 'occupancy_model_30min.pkl'
DEFAULT_CONFIG_PATH = 'occupancy_model_30min_config.json'
...
def predict(
    ...
    Returns one row per usable timestamp with:
        timestamp, forecast_time, occupancy_now, occupancy_forecast_30min
    plus, if the upload happens to contain enough trailing history:
        occupancy_actual_30min_later   (kept ONLY for optional
    ...
    # populated. headcount_target_30min is allowed to be NaN here --
    ...
    result = pd.DataFrame({
        ...
        'occupancy_forecast_30min': predictions,
    })
    result['occupancy_forecast_30min'] = result['occupancy_forecast_30min'].clip(lower=0)

    if TARGET_COLUMN in usable.columns:
        result['occupancy_actual_30min_later'] = usable[TARGET_COLUMN].values
