...
class HVACConfig:
    ...
    prediction_horizon_min: float = 30.0
...
def run_1r1c_strategy(sim_data: pd.DataFrame, strategy_name: str, cfg: HVACConfig) -> pd.DataFrame:
    ...
    occupancy_forecast_30min, outdoor_temperature_C
    """
...
def run_simulation(occupancy_df: pd.DataFrame, cfg: HVACConfig = None, outdoor_temp_column: str = 'outdoor_temperature_C'):
    ...
    occupancy_df : pd.DataFrame
        Must contain:
          ...
          - occupancy_forecast_30min   : forecast occupancy N minutes
    ...
    required = ['timestamp', 'occupancy_now', 'occupancy_forecast_30min']
    ...
    sim_input['occupancy_forecast_30min'] = pd.to_numeric(
        sim_input['occupancy_forecast_30min'], errors='coerce'
    ).clip(lower=0)
    ...
    # If the caller had ground-truth "actual occupancy 30 min later"
    # (e.g. a historical backtest), carry it through for validation
    # plots -- purely informational, never used by the controller.
    if 'occupancy_actual_30min_later' in occupancy_df.columns:
        comparison_df['occupancy_actual_30min_later'] = pd.to_numeric(
            occupancy_df['occupancy_actual_30min_later'], errors='coerce'
